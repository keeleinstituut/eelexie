...
# SHS::ankeet_uuenda_faili_formaat();
...
if ( $q->param('userchg') eq '1' ) {
    ...
    if ($logirida =~ /\->/) { # ehk siis et kui midagi muudeti
	foreach $a (@ankeedid) {
...
sub loe_peatoimetajad {
    ...
    # initsieerime peatoimetajad grupiga DEV, meili pole vaja
    foreach (@grupid_too) {
	next unless /^grp_DEV: /;
	$_ = $';
	foreach $pt (split(/ /, $_)) { $ptd{$pt} = 1; }
    }
...
sub kirjuta_peatoimetajatele {
    my %xptd = ();	# iga kord kui siia poordutakse, teeme PTD grupi uuesti
    my ($pt, $ptmail) = '';
    ...
    foreach my $id (keys %xptd) {
#print "$id = ";
	$pt = kasutaja_meiliaadress($id);
#print "$pt; ";
	next unless $pt;
	$ptmail .= ', ' if $ptmail;
	$ptmail .= $pt;
    ...
    print "<a href=\"mailto:devnul\@eki.ee?bcc=", $ptmail, "\&subject=EELexi_peatoimetajate_list\">Kirjuta peatoimetajatele</a>";
}
...
sub kasutaja_meil {
	...
	$s .= "Teie konto EELexi s\x{f5}nastike s\x{fc}steemis on aktiveeritud. ";
...
sub prindikasutajaandmed {
	...
	if ( on_selle_sonastiku_peatoimetaja ($kasutajanimi, $id) ) {
	    $snimi = '<b>' . $snimi . ' (PT)</b>';
	}
	print "<tr>";
	print "<td>$id</td>";
...
sub meiliteade {
    ...
    print (M "Content-type: text/plain; charset=utf-8\n\n");
    print (M $teatesisu . "\n");
...
sub prindikasutajad {
    ...
    print "<table>\n";
    print "<tr><th>Kasutajanimi</th><th>Saab kirju eelex\@eki.ee</th><th>Uue s\&otilde;nastiku lisamis\&otilde;igus</th><th>Peatoimetajate grupis</th></tr>\n";
	...
	print "<td>";
	print "<form action='admhalda_proc.cgi' name='${kk}K'><input type='hidden' name='kasutaja' value='$kk'>\n";
	$chkd = (SHS_access::SHS_user_in_group ($kk, 'grp_KIRJAD', $grpfile_too) ? 'checked' : '');
	print "<input type='hidden' name='gkirjad' value='1'>";
	print "<input type='checkbox' $chkd id='kirjad' name='kirjad' onChange='this.form.submit();'>";
	print "</form>\n";
	print "</td>";

	print "<td>";
	print "<form action='admhalda_proc.cgi' name='${kk}L'><input type='hidden' name='kasutaja' value='$kk'>\n";
	$chkd = (SHS_access::SHS_user_in_group ($kk, 'grp_LISAJAD', $grpfile_too) ? 'checked' : '');
	print "<input type='hidden' name='glisajad' value='1'>";
	print "<input type='checkbox' $chkd name='lisajad' id='lisajad' onChange='this.form.submit();'>";
	print "</form>\n";
	print "</td>";

	print "<td>";
	print "<form action='admhalda_proc.cgi' name='${kk}P'><input type='hidden' name='kasutaja' value='$kk'>\n";
	$chkd = (SHS_access::SHS_user_in_group ($kk, 'grp_PTD', $grpfile_too) ? 'checked' : '');
	print "<input type='hidden' name='gpeatoimetajad' value='1'>";
	print "<input type='checkbox' $chkd name='peatoimetajad' id='peatoimetajad' onChange='this.form.submit();'>";
	print "</form>\n";
	print "</td>";
