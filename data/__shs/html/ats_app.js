...
// -----------------------------------------------------------------------------------
function dirStartOperation(oPrmDom)
{
  var cmdId;
  cmdId = oPrmDom.documentElement.selectSingleNode("cmd").text
  var axpNode, art_xpath
...
// -----------------------------------------------------------------------------------
function dirQueryResponseAsync(oPrmDom)
{
  xmlHTTPAsync = getXmlHttpObject();
  xmlHTTPAsync.onreadystatechange = dirDoHttpReadyStateChange;
  xmlHTTPAsync.open("POST", "srvfuncs.cgi", true);
  xmlHTTPAsync.setRequestHeader("Content-Type", "text/xml; charset='utf-8';");
  // cmd, vol, nfo, axp, exp[, q : A]
  xmlHTTPAsync.send(oPrmDom.xml);
}
// dirQueryResponseAsync
...
// -----------------------------------------------------------------------------------
function dirDoHttpReadyStateChange()
{
  if (xmlHTTPAsync.readyState == XMLHTTP_COMPLETED)
  {
    var oSrvRspDOM, rspNode, domsta;
    ...
    oSrvRspDOM.setProperty("SelectionLanguage", "XPath");
    oSrvRspDOM.setProperty("SelectionNamespaces", sXmlNsList);
    dirAsyncCompleted(oSrvRspDOM);
  }
}
// dirDoHttpReadyStateChange
// -----------------------------------------------------------------------------------
function atsAsyncCompleted(objXMLDom)
{
  // at ParseSrvInfo(objXMLDom);
  atsParseMyInfo(objXMLDom,1);
  StopOperation();
}
// dirAsyncCompleted
// -----------------------------------------------------------------------------------
function dirAsyncCompleted(objXMLDom)
{
  // at ParseSrvInfo(objXMLDom);
  atsParseMyInfo(objXMLDom,2);
  StopOperation();
}
// dirAsyncCompleted
...
// showArtLink
// -----------------------------------------------------------------------------------
function atsParseMyInfo(oRespDom,nVL)
{
...
//alert(zNzimi);
 ...
 // var xartGuid = zG;
 // var xvolId = zVol;
 // var xsCmdId = "BrowseRead"; //'sCmdId - globaalne;
 // var xsQryInfo = zNzimi; //sQryInfo - globaalne;

 // sPrmDomXml = "<prm>" + "<cmd>" + xsCmdId + "</cmd>" + "<vol>" + xvolId + "</vol>" + "<nfo>" + xsQryInfo + "</nfo>" + "<G>" + xartGuid + "</G>" + "<qM>MySql</qM>" + "</prm>";
 // oPrmDom = IDD("String", sPrmDomXml, false, false, null);
 // atsStartOperation(oPrmDom);
 // ================================================================
...
// Ühe väärtuse otsing
 // ================================================================
    var zzzRet ="";
    var xartGuid = sParms[3];
    var xvolId = sParms[2];
    var xsCmdId = "BrowseRead"; //'sCmdId - globaalne;
    var xsQryInfo = sParms[0]; //sQryInfo - globaalne;

    sPrmDomXml = "<prm>" + "<cmd>" + xsCmdId + "</cmd>" + "<vol>" + xvolId + "</vol>" + "<nfo>" + xsQryInfo + "</nfo>" + "<G>" + xartGuid + "</G>" + "<qM>MySql</qM>" + "</prm>";
    oPrmDom = IDD("String", sPrmDomXml, false, false, null);
    dirStartOperation(oPrmDom);
 // ================================================================
      }
    }
    else
...
// -----------------------------------------------------------------------------------
function dirXmlValue(oMyXmlDom, zznt, zzvol)
{
  var i, zNzimi,cNzimi, zAttrHom, zArtElemCnt, zVol, zG, zzField;
  var tblAjalugu, rida, veerg, artGuid;
  var zzArtElements = oMyXmlDom.documentElement.selectNodes("..//A");
  ...
  ccCount = zznt;
    ...
    //zzzRet = zzField + PD + zAttrHom + PD + zzvol + PD + zG;
    //alert(zzzRet);
        ...
        if (zzField != "")
        {
          ...
          ccField = zzField;
          selectedNode.text = zzField;
          if (zAttrHom != "")
          {
            setMyNSAttribute (selectedNode, DICURI, DICPR + ":i", zAttrHom);
            //selectedNode.setAttribute(DICPR + ":i", sParms[1]);
          }
          else
          {
...
// -----------------------------------------------------------------------------------
function atsViitOnOff() {

  if (ccOnOf == 1){
     ccOnOf = 0;
     alert("Viitekontroll on väljalülitatud(OFF)!")

  }else{
     ccOnOf = 1;
     alert("Viitekontroll on sisselülitatud(ON)!")
  }
  return;
}
// atsSplitMS
