...
function SetEdits() {
    ...
    if (mT == "") {
        if (snQName == "n:aa") { //'Eesti kohanimeraamat: kirjandus; teos: autor aasta;
            ...
            mT = "pikkLoend";
        } else if (snQName == "x:aut") { // ets: standard
            mT = "pikkLoend";
        ...
        if (!(nsv == "" && scv == "")) {
         ...
         if (ccOnOf == 1){
            ...
            if (atsOkRun(dic_desc) == 1){;
              ...
              ccField = ccField.replace(/\s+$/, '');
              if (atsYesSymbString(ccField) == 0) {
                //ccField = ccField.replace(/\W/g, '');
                  ccField = atsNoDig(ccField);

              //alert ( currTekst );
              }
			  var currTekst = oBlur.value;
              currTekst = currTekst.replace(/\s+$/, '');
              if (atsYesSymbString(currTekst) == 0) {
                 //currTekst = currTekst.replace(/\W/g, '');
                   currTekst = atsNoDig(currTekst);

                 //alert ( currTekst + " == " + ccField );
              }
             // selectedNode.text = "Viite kontroll. Oota vastust!";

			  if (ccField != currTekst) {
                alert ("Viite kontroll. Oota vastust!");
...
function lisaVeelArtToolse() {
	...
	// ======================================================================================================
	// ats+++  (22.01.2013/22.01.2013)
   	// ======================================================================================================

       if (sArtToolsMenu) {
          sArtToolsMenu = sArtToolsMenu + "<tr class='dl'><td>" + delimline + "</td></tr>";
       }
       // Viitekontrolli sisse ja väljalülitamine
       sArtToolsMenu = sArtToolsMenu + "<tr class='mi' id='idViitKontr' value='idViitKontr'><td>Viitekontroll ON/OFF</td></tr>";
