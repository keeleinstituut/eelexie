...
//      retStr = currentLn + '->' + ln + ':' + myNodeList[0].nodeName + ':' + retStr;
  ...
  <xsl:variable name="itad">;tmnr;tmen;trm;</xsl:variable>
  ...
  <xsl:template match="trmd">
    <html>
      <head></head>
      <!--<title>Indeks (et)</title>-->
      <!--<meta HTTP-EQUIV='Content-Type' CONTENT='text/html; charset=utf-8'></meta>-->
      <body lang="et" style="font-size:xx-small;">
        <xsl:apply-templates select="trm">
          <xsl:sort select="@O" lang="et"/>
        </xsl:apply-templates>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="trm">
    <p>
      <xsl:value-of select="al:RS(., '1')" disable-output-escaping="yes" />
    </p>
  </xsl:template>
  <!-- A -->
